...
3. Validate the design contract and all source bindings with the sibling analysis skill's `scripts/design_contract.py`. Module `specRef` and `recipeRef` are exact versions, not suggestions; numeric parameters are resolved by the project registry. A Figma node link is traceability, not proof of a current exported snapshot. Use [visual presentation rules](../build-nextgame-umg/references/visual-presentation-rules.md) to identify concrete compatibility gaps in ownership, text geometry, state scope and art evidence; do not reinterpret locked decisions or invent missing visual parameters.
...
For `formal-art`, `upgrade-art`, or `local-art`, preserve the same coordinator and task identity and continue through sibling `$refine-nextgame-ui-art`; see [the art-stage handoff](../build-nextgame-umg/references/on-demand-art-stage.md). The art contract adds resource correspondence and presentation decisions without duplicating locked layout/state/collection authority. If art needs a locked design change, revise the design-contract source, compile a new pending Requirement, and accept that revision before application. Do not patch the compiled Requirement or infer a new design from the art preview. A verified development baseline alone does not complete a formal-art request.

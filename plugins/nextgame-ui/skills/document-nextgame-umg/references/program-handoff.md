...
4. `ui-build-acceptance.json`. This is authorization evidence, not a fourth source of UI meaning or actual implementation facts. It must have phase `post-build-ui-review`, status `accepted`, exact hashes and identities for inputs 1-3, and exact reviewed asset ID/path coverage. Version 0.1 requires direct-user-message provenance from a user reviewer. Explicit version 0.2 requires a one-request user grant and truthful post-result primary-coordinator review, with a single frozen-result consumption record; see [delegated result acceptance](delegated-result-acceptance.md).

Validate with the schemas in `assets/` and the deterministic scripts in `scripts/`. A planned layout or Bundle field cannot substitute for actual Unreal evidence. The documentation Agent must never create, infer, repair, or pre-authorize acceptance. Under 0.1, requirement review and pre-result requests cannot substitute for later direct-user confirmation. Under 0.2, only a separately validated explicit one-request delegation permits the primary coordinator to perform the actual review after the final result; an ordinary full-workflow request does not qualify.

Any asset mutation or re-save, Bundle change, preview replacement or re-verification, or Readback change invalidates the prior acceptance. Return to the build workflow, regenerate the affected final artifacts and present the revised result. A consumed delegated grant cannot transfer to the new result: obtain new specific direct-user authority or ordinary post-result confirmation. Prototype, legacy standalone, and explicitly no-document tasks do not enter this contract and do not create acceptance.

## Optional art evidence in the final Bundle

Bundle 0.4 requires `artStage` with `goal`, `request`, `plan`, and `verification`; the last three are `{path, sha256}` bindings relative to the Bundle. Paths and current file hashes are validated, then the passed art verification is revalidated. Normalized Readback 0.4 is the 0.3 actual identity shape with empty `reuseRelations` permitted. Old versions retain their original strict shapes.

The art acquisition snapshot carries visual properties separately. At the document gate, `validate_unreal_widget_readback.py` passes the current normalized readback into the art validator to check structural identity and freshness against that snapshot. The final Bundle, art evidence, normalized readback, and versioned post-result acceptance must belong to the same completed result. Art data is verification evidence, not an additional source of runtime API intent. See [the art-stage handoff](../../build-nextgame-umg/references/on-demand-art-stage.md).
...
The default presentation layer is the neutral, built-in NextGame UMG document template at `../../artifact-template-nextgame-umg/assets/reference.docx`. Read [program-document-template.md](program-document-template.md) before generating a document. The template controls page geometry, styles, section order, tables, asset-detail blocks, and pagination; it never supplies production facts. Replace every placeholder from the verified handoff and generated content contract, retain one concise programmer-facing `程序范围` callout, omit empty conditional sections, renumber the remaining sections continuously, and never carry example-system text into the output. Do not print `内容模式`, a restatement of the user's request, AI workflow notes, format-tuning commentary, or other generation-process explanations in the opening. Do not create a standalone target-asset section or target-asset table; keep each asset path only in its matching asset-detail block. Title each asset-detail block with the exact Blueprint basename immediately followed by a concise Chinese functional name, for example `uw_fight_use道具使用图标`, without parentheses or process/role labels such as `近期子控件` or `主目标`; do not output a `根控件` field. Keep the asset block order exact: title → `资产路径` → `Parent Class` → `功能说明` → merged WidgetTree/program-purpose table → `程序接入关系` → owned collection, state, and support-dependency content. Derive the concise Chinese name and functional explanation only from verified programmer-facing relationships.

Route document-visible collections, state models, and support-dependency notes by their owning or hosting asset without changing their handoff semantics. If that asset is present in the first `资产详细说明` module, place the content inside its asset-detail block after the WidgetTree and programmer-facing relationships; do not emit standalone document-level collection, state, or `支持依赖定位` sections. Render embedded collection and state identifiers such as `collection.teammate.entries` and `state-model.teammate-life` as ordinary body text or an inline label, never with Heading, `Tool Asset`, or section-title styling. Do not repeat `所属资产` or the asset path inside an embedded collection or state block. Only unmatched collection, state, or support-dependency content whose asset is absent from the first module may enter a conditional `其他资产程序说明` module. That module contains only unmatched items, groups them by asset, records the asset identity and complete path once at group level rather than in every item, and is omitted when empty. Number all subsequent global sections continuously.

The asset-detail table combines navigation and program-purpose lookup without changing their separate evidence semantics. The deterministic `program-document-content.json` 0.4 contract projects each asset's verified `parentClassPath` plus ordered node rows from the validated readback fields `widgetName`, `classPath`, `parentWidgetName`, and `isVariable`, then joins `programPurpose` only from the same handoff asset's `programVariables[].widgetName`; `$documents` receives those structured values, never raw readback or diagram files. Render every asset as one native Word table with the exact columns `层级 / Widget`, `Class`, `Is Variable`, and `程序用途`, one actual node per data row. Set the first-cell Word left indentation to `depth × widgetTreeTables.indentTwipsPerDepth` (currently `180 twips` per level), serialize `Is Variable` exactly as lowercase `true` or `false`, and write an empty fourth cell for a node that is not a program variable. A blank purpose never changes the actual third-cell value, and a purpose never crosses asset boundaries. Use fixed `52 / 34 / 20 / 40 mm` columns with matching `tblW`, `tblGrid`, and `tcW`; repeat and keep the header together, keep every data row together, and do not emit any `w:trHeight`. ASCII connectors, space-padded pseudo-trees, PNGs, screenshots, drawings, and other image substitutes are forbidden. A `reuse-only` asset with no owned WidgetTree nodes uses one data row merged across all four columns containing exactly `无自有 WidgetTree 节点（继承结构不重复列出）`. Do not project saved Visibility or other static Designer properties into the table or prose, and never emit a separate document-level program-variable section.

Before `$documents` generates the DOCX, run `prepare_program_document_contract.py` with the exact current `--requirement`, `--bundle`, `--readback`, and `--build-acceptance` paths. It reruns the complete current-source and acceptance gate, rechecks the handoff binding, binds the verified handoff hash, and emits the complete identifier inventory plus canonical semantic relationship statements. Comparing only an old handoff with an old acceptance is not sufficient after any source file changes. Retain `requiredSemanticRelationshipStatements` in the content contract and machine-side verification artifacts for deterministic relationship auditing, but do not copy them into the human-readable DOCX and do not create Appendix A or a semantic-trace appendix.

Keep Readback snapshot timing, acquisition details, source bindings, and provenance boundaries in machine contracts and evidence only. Do not emit a developer-facing `只读快照边界` section. The programmer-facing document contains only the runtime integration information that helps implementation.

Newly generated `program-document-content.json` and `document-verification.json` contracts use version `0.4`; final verification binds the content-contract hash and the normalized four-column asset-detail table summaries. Existing version `0.2` verification artifacts remain revalidatable through their legacy text-coverage path, and existing version `0.3` contracts remain revalidatable through their original three-column table path; new verification output cannot be downgraded. `render-evidence.json` remains version `0.1` because its rendering provenance and page shape are unchanged.

After `build_program_docx.py` revalidates the full current source chain and deterministically generates the DOCX, create `render-evidence.json` in a fresh empty page directory. The builder fixes the neutral reference, output environment-variable name, and Handoff filename; consumes only verified Handoff/content fields as document business data; emits canonical byte-stable OPC packaging; and leaves an existing byte-identical file untouched. It never replaces fresh rendering. The validator performs the authoritative headless conversion itself: current DOCX to a fresh canonical PDF, then that PDF to final `page-<N>.png` files through `pdftoppm`. The evidence binds the current DOCX SHA-256, executable LibreOffice and Poppler versions, the canonical PDF, render time, and the hash, byte size, and dimensions of every page. `document-verification.json` then binds that evidence, the handoff, the DOCX, the machine-side semantic statements, and the explicitly reviewed pages. Final validation reconverts the current DOCX and rerenders every page before comparison. It requires:
...
- machine-contract retention and deterministic binding of every generated semantic relationship statement, including target asset paths, each state branch identity and default flag, and every accepted explicit `State outcome` target Visibility; these statements are not required to appear in DOCX text, and saved Designer/readback Visibility values remain excluded;

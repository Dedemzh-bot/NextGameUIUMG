...
- Make the Button's effective hit area cover the intended interactive region. Put its own visual Panels, GameImages, and TextBlocks below its content layout, including their accepted state branches; keep them passive. Do not flatten a Button's decoration into sibling nodes simply to match the reference position.
- Preserve semantic ownership independently from drawing order. A region heading and another help Button stay under their respective accepted owners; proximity to a large Button is not evidence that they belong inside it. Check both actual parentage and effective draw order before moving a node.
- A Button has one direct content child. A Canvas content child must explicitly declare its direct-parent `buttonSlot`: use zero Padding and Fill/Fill when it should cover the entire Button, and preserve explicit accepted nonzero Padding when the design requires an inset. The current layout contract supports Fill/Fill only; a different alignment needs supported contract lowering before execution, not an invented property or silent normalization. An explicitly declared supported Button Slot on another child (for example, an Overlay with a real layering duty) is also lowered and read back; a historical omitted Slot is not silently assigned one.
- In requirement-driven builds, preserve the nearest accepted Button ancestor in `parentElementId` through concrete node mappings and declared instance boundaries. An explicit shared/inherited realization is verified through its existing relationship contract, not by inventing a node.
...
- For region-centered text, use a bounded width with `Size To Content` disabled so Center justification has real space, or use content-driven width at the declared center with matching point anchors and X alignment `0.5`. For the owner's geometric center, both X anchors are `0.5`; retain an explicitly designed offset center rather than guessing it from Justification. Right-aligned growth similarly needs a bounded text area or a declared right point and X alignment `1`.
- Do not combine an auto-sized label with a left-origin Slot and assume Center justification will center the component in a wider region. Check anchor minimum/maximum, alignment, offsets, width, auto-size and wrap width jointly. A deliberate local text origin is not invalid merely because its text is centered internally.
- For dynamic single-line names or values, test short and representative long values. If the protected edge must remain fixed, use the matching point anchor and alignment plus content-driven width so the text expands away from that edge.
- Review every text effect separately using [Text effects and annotations](visual-presentation-rules.md#text-effects-and-annotations); justification and outline appearance are independent checks.
...
- For deliberately bounded wrapping text, the explicit node `textCapacity` contract `bounded-wrap/1` may retain `autoSize: false` in a point-anchored fixed Canvas slot. Capacity dimensions must exactly match its positive Slot and rectangle dimensions, `Wrap Text At` must be positive and no larger than the width, and the contract must carry its accepted evidence. This is a capacity declaration, not a claim that text fits; verify actual short and long text after rendering. Without this opt-in, the existing expanding-text rule is unchanged.
- Preserve deliberate fixed margins while stretching. Do not replace every offset with a fixed absolute width or height.
- Verify with longer representative text and confirm that text, entry height, background, and decoration still agree. For joined headers/bodies and common popups, follow [Shared surfaces and visible seams](visual-presentation-rules.md#shared-surfaces-and-visible-seams): use one width authority, account for visible resource insets and test long-title/short-body as well as short-title/long-body cases.
...
- An accepted exact-width/natural-height constraint can use the explicit `sizeBoxConstraints` version 1 contract and a Fill `sizeBoxSlot`. Its planned image/Auto-flow lower bound may propagate through the unconstrained axis; it does not become an actual measured Desired Size. Follow the native setter, clear, and post-save readback requirements in the SizeBox configuration contract.
...
### Exact-axis SizeBox execution and planned dependency

- For the executable UILayoutSpec exact-axis contract, use node-level `sizeBoxConstraints: {"version": 1, "widthOverride": 456, "heightOverride": null}`. The object is closed and both axes are required: a positive finite number enables that exact override via its native setter; `null` clears it. At least one axis must be enabled. Version 1 explicitly clears all minimum/maximum desired-size and aspect-ratio bounds; it cannot represent those bounds, zero overrides, or inferred enable states.
- Its sole child declares a closed `sizeBoxSlot` with `padding: [left, top, right, bottom]` (nonnegative finite values), `horizontalAlignment`, and `verticalAlignment`. Set both alignments to `Fill` for the fixed-width, natural-height contract. The planner lowers these actual SizeBoxSlot properties through the official list/get/set sequence.
- The current official tool registry exposes SizeBox properties but no native setter invocation. `prepare_build.py` emits an explicit `native_setter_fallback` operation with `contract: size-box-constraints/1`, the real NxUE method calls, the widget reference, and `expectedProperties`. This is runner metadata, never a fabricated official tool. The standard executors reject the mixed plan before any Editor call; an authorized runner must execute the fallback in sequence and compare official bound values/enable bits before compile and after save. Never skip the operation or claim completion from its planned values.
- `contentSizeProof` may propagate the existing conservative image/Auto-flow lower bound through this one-child constraint when the SizeBox's own Canvas slot is auto-sized and its child slot is explicit Fill with zero padding. Enabled axes replace the lower bound; cleared axes inherit the child's lower bound. Nonzero SizeBoxSlot padding is lowered explicitly but is outside this conservative proof. Unknown or collapsed sources, cycles, unsupported containers, missing children and fabricated zero sizes cannot establish the proof. This remains `layout-dependency/1` planning evidence, never verified Slate measurement.

The distinct opt-in `layout-dependency/2` supports fractional fixed width and horizontal Fill text within a real fixed-width SizeBox subtree. It preserves the version 1 proof and requires its own strict complete-subtree width propagation and natural-height lower bound; see [Fixed-width natural-height dependency v2](layout-spec.md#fixed-width-natural-height-dependency-v2). It is planned dependency evidence, never actual Slate measurement.

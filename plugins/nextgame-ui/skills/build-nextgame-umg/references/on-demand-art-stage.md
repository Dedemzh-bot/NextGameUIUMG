...
## Development baseline property deferral

`development-baseline/2` is an explicit alternative for formal-art when accepted default Brush resource, DrawAs or mirroring values differ from actual GameImage Brush properties. It preserves the original pending `key-properties` checks and proves each deferred field against the current accepted Requirement, Bundle node mapping and actual art snapshot. Version 1 remains preview-only, and the two capabilities cannot coexist. Neither enables a pending final Bundle or document gate.

Each asset still needs passed schema, compile, save, WidgetTree and explicitly identified basic property checks, with actual execution receipts and post-save normalized readback. The exact deferred check identity and all resource gaps are retained, with a hash-bound primary review performed after inspection. The validator does not author this review. Never convert a property check to preview or mark it passed merely to enter art.

Explicit unresolved text-effect, nine-slice and procedural material source responsibilities stay in `pendingArtJudgments`, tied to original pending preview closure checks. They are pending art decisions, not confirmed native values; default zero or empty effects do not complete them. Version 2 only defers these three accepted resource-related Brush differences. Actual identity, variables, CDO modes, state/EntryClass, Slots, fonts and geometry cannot use that exception. The final Bundle must retain and pass every original obligation. See [the exact contract and migration](../../refine-nextgame-ui-art/references/workflow.md#development-baseline-resource-deferrals).

The baseline remains running/pending with built assets; completed compile/save and normalized actual-state checks are mandatory for every asset. Version 1 permits only pending visual previews; version 2 additionally permits its strictly validated resource-property deferrals. The common source and actual-state validation cores are shared with the final document validator; no error filtering or arbitrary skip flag selects this path. The normal final readback entry point always adds its original final lifecycle and art completion gates.

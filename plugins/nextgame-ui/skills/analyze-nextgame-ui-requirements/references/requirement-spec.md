...
Describe candidate `screen`, `child-widget`, and `list-entry` assets with `assetPath`, `referenceSize`, `layoutSpecPath`, dependency IDs, zero-based contiguous `buildOrder`, scope, coverage IDs, evidence, claims, an inventory `boundaryClassification`, and a `designSizeModeDecision`. `assetKind` classifies semantic/build responsibility only and never chooses the Designer mode. The target Blueprint basename has one explicit project exception: `umg_*` is always `FillScreen` and is not evidence-analyzed. Only `uw_*` is analyzed for `Desired` versus `FillScreen`; nonstandard legacy basenames use the conservative `FillScreen` fallback.

The closed decision contains `mode`, `basis`, `reason`, `evidenceIds`, and one proving `claimId`. That claim must exist in the enclosing asset's `claimIds`, use `type: asset-decomposition`, include the asset ID in `subjectRefs`, and cover every decision evidence ID in its own `evidenceIds`. In an accepted Requirement, the existing review gate and in-scope claim rules therefore make this proof a reviewed conclusion rather than an arbitrary evidence citation.

- `project-umg-rule` is valid only for a basename beginning `umg_` and requires `FillScreen`. Its decision `evidenceIds` stay empty because no per-interface analysis is performed; its proving claim must explicitly name `project-umg-rule` and `FillScreen` and cite evidence whose source is `kind: project-rule`;
- `viewport-filling` means the analyzed Widget fills a viewport/screen host and requires `FillScreen`;
- `content-sized-local` means positive evidence proves a locally hosted control whose root content produces a non-zero Desired Size, and requires `Desired`;
- `verified-reference` means an explicitly designated existing Widget's actual Designer/hosting behavior was inspected; it may support either canonical mode, but at least one decision evidence item must have `measurementMethod: editor-readback` and resolve to a source whose `kind` is `project-asset` and whose `locatorKind` is `unreal-object`. The normal Unreal-source rules then also require a `/Game/...` path, relative snapshot, and content hash; an inline project-asset record cannot stand in for Editor readback;
- `fallback-unclear` records that the available evidence cannot prove a local content-sized contract and therefore requires the tolerant default `FillScreen`. Its decision evidence may be empty, but the proving claim statement must explicitly contain the canonical `fallback-unclear` and `FillScreen` tokens so the reviewed fallback cannot be confused with an evidence-backed conclusion.

Every analyzed non-fallback `uw_*` decision and every `Desired` decision has at least one canonical evidence ID, and those IDs also belong to the enclosing asset plan. A `uw_*` asset cannot use `project-umg-rule`; when its analysis is unclear it uses `fallback-unclear` with `FillScreen`. A basename outside both conventions is treated as legacy and must also use `fallback-unclear` with `FillScreen`, never `Desired`. Canonical modes are only `FillScreen` and `Desired`; `DesiredOnScreen`, `Custom`, and `CustomOnScreen` are not substitutes. The source image size and `referenceSize` remain independent geometry contracts. Allowed boundary classifications are `screen-root`, `entry-widget-class`, `runtime-template`, `reusable-widget`, `statically-referenced` (reserved for explicit system rules such as Fight), and `stale-candidate`. Normal non-Fight regions and screen-local collections remain ordinary semantic Panels/leaves in the screen; do not extract a functional wrapper without an accepted boundary claim and evidence. A list-entry that is positively analyzed as `Desired` must materialize a non-zero local Desired Size in UMG: the first internal structural Panel Slot has explicit non-zero width and height matching its local `referenceSize` (or a separately verified content-driven desired size). A zero-offset full-stretch root is invalid for that case, and a SizeBox must not be introduced solely to supply the root size. A pending spec may leave the array empty. In an accepted spec, every in-scope plan and every claim attached to it must be accepted; proposed, unresolved, or rejected decisions remain out of scope with `scopedOutReason`.

...
All nine analysis roles run from validated, content-addressed role packets rather than inherited conversation history. Generate them with [prepare_agent_inputs.py](scripts/prepare_agent_inputs.py), validate each packet before dispatch, and spawn the role with `fork_turns: "none"`, no model or reasoning override, and a prompt that contains only the packet path. The packet embeds its self-contained role card and output contract. Never add chat messages, another Agent's output, or an expected conclusion to that prompt.
...
Before project-pattern discovery, fully validate the immutable Registry source already copied into this request. Build a compact discovery artifact with `python ../../scripts/shortlist_shared_widgets.py --registry <immutable-registry.json> shortlist <query flags> --output <request-root>/inputs/shared-widget-shortlist.json`. The artifact contains exactly one non-executable card for every Registry entry; soft matching only ranks/classifies and never drops an entry. Missing, zero, ambiguous, or unmatched queries are explicit fallbacks that retain all cards. A positive reuse conclusion requires on-demand full expansion with `expand <entry-id> --shortlist-json <shortlist.json> --output <expanded-entry.json>`; expansion regenerates the shortlist against the fully validated Registry and rejects stale or forged bindings, but remains non-executable because it only hydrates details. The accepted Requirement size/state/semantic checks and final Bundle plus full Registry validation alone authorize execution. If shortlist creation or expansion cannot be validated, use the complete immutable Registry and preserve the uncertainty; never conclude that reuse is absent from a failed shortlist.

Generate discovery packets in one deterministic call, attaching the shortlist only to the project-pattern role:

```text
python scripts/prepare_agent_inputs.py prepare <request-packet.json> --output-dir <request-root>/agent-inputs --role visual-structure --role text-requirements --role project-pattern --role-attach project-pattern:registry-shortlist=<shortlist.json>
```

Run `prepare_agent_inputs.py validate <packet.json> --request-root <request-root> --request-packet <request-packet.json>` for each emitted discovery packet. The RequestPacket path is validator-side authority and is not exposed inside the Agent-facing packet. The discovery packets enforce exact source isolation: image only for `visual-structure`, user text only for `text-requirements`, and project-rule/project-asset sources only for `project-pattern`.

### 2. Run round-one independent discovery

Spawn these three agents in parallel from their validated packet paths, each with `fork_turns: "none"`:
...
Write the full authoritative normalized context to `contexts/normalized-context.json`, then generate the three focused packets with `--context <that-file>` and attach the Registry shortlist only to `asset-decomposition`. `prepare_agent_inputs.py` copies semantic JSON values without summarizing them and emits a role-specific context projection under `contexts/roles/`. It removes only the normalizer-only trace and explicitly irrelevant planning lists. Unknown top-level semantics, an unreviewed field inside a section that would be omitted, or a reference that would dangle only after omission produces a compact `full-fallback` copy instead of narrowing. A missing/invalid base section, duplicate canonical ID, unreviewed reference-like field, request/digest mismatch, or already dangling base reference is invalid authority and stops packet generation. Validate every context-aware packet with `prepare_agent_inputs.py validate <packet.json> --request-root <request-root> --request-packet <request-packet.json> --base-context <contexts/normalized-context.json>` before dispatch; the full base path remains a validator-side sidecar and is not exposed to the Agent.
...
Round-two agents also emit `AgentFindings`. They may add or challenge claims but may not edit the authoritative normalized base or another agent's output. Dispatch each validated role packet with `fork_turns: "none"`; the packet binds its own projected or `full-fallback` context. Each findings file keeps the RequestPacket `inputDigest`, sets `contextDigest` to the canonical hash of that role context, and binds every finding through `subjectRefs` to existing canonical IDs in that context. Validate with `python scripts/validate_agent_findings.py <findings.json> --request-packet <request-packet.json> --context <role-context.json> --base-context <contexts/normalized-context.json> --role-packet <role-packet.json>`. The validator externally rechecks the projection against its full authoritative base, so a projection never replaces or discloses that base.
...
For every newly synthesized requirement, write `analysisPolicy: { "geometryEvidenceRequired": true, "listPriorityRequired": true, "assetBoundaryRequired": true, "standardSystemBoundaryRequired": true, "stateControlInputRequired": true, "staticVisualCoverageRequired": true, "imageCompositionRequired": true, "explicitPanelSlotsRequired": true, "explicitImageOwnerIntentRequired": true, "designSizeModeRequired": true, "noHistoryRolePacketsRequired": true }`. Standard non-fight screens default to semantic Panels and screen-local Lua collections; a `uw_*` boundary requires one of the four permitted boundary reasons plus evidence. The no-history flag additionally requires the full normalized-base binding and all nine findings-to-role-packet ref/hash bindings to pass strict final validation. These flags make the geometry, repeated-family, asset-boundary, high-level state-control-intent, static-visual coverage, complete-image composition, exact owner-adaptation binding, reviewed immediate-parent Slot, per-asset evidence-driven Designer Screen Size, and isolated-agent provenance gates machine-enforceable while preserving validation compatibility for pre-policy archived requirements.
...
Run three independent review passes over the draft: state/visual adversarial review, schema/reference/buildability review, and requirement-to-node coverage review. Before dispatch, freeze the pending Draft as an immutable validator-side authority, then regenerate their no-history packets with the same full context, `--include-review --draft-requirement <immutable-pending-draft>`, and any coverage artifact. This creates and binds exactly three role-specific files under `review-views/`: `state-visual-review.review-view.json`, `schema-feasibility-review.review-view.json`, and `coverage-review.review-view.json`. The complete Draft is never bound into a reviewer packet. The `--draft-requirement` argument is retained only so the builder and validators can hash the authority, reproduce the expected View, and reject a forged or stale binding.

Each Review View is a non-authoritative, exact-value projection. It selects reviewed roots and then computes the transitive dependency closure by canonical ID; every referenced canonical object must enter the same View. It is not a field-deletion shortcut. Unknown fields or unregistered reference-like semantics, duplicate definitions, dangling references, or an incomplete closure force an exact `full-fallback` whose embedded Requirement equals the immutable pending Draft. Never treat content omitted from a projected View as absent from the complete Requirement. The Draft must match the Request identity and original text, remain pending, and have no structural Schema errors other than unknown fields before any View is emitted. Unknown fields are allowed only to trigger the exact fallback and must be removed or formally supported before final full-Requirement validation.

Validate each generated review packet against both hidden authorities before dispatch:

```text
python scripts/prepare_agent_inputs.py validate <review-role-packet.json> --request-root <request-root> --request-packet <request-packet.json> --base-context <contexts/normalized-context.json> --draft-requirement <immutable-pending-draft>
```

A `--coverage-evidence` input must pass [visual-coverage-report.schema.json](assets/visual-coverage-report.schema.json), bind an immutable image hash in the same RequestPacket, and have internally consistent unresolved counts, status, and gate; a label plus file hash alone is insufficient. Each reviewer receives only its own validated packet path and runs with `fork_turns: "none"`; it never receives a separate full-Draft binding or inherits other review findings. The coverage reviewer additionally receives the validated image/coverage evidence and must compare the raster overlay, every medium/high candidate crop, and an overlapping full-image contact sheet against canonical static-image elements, family/entry roots, state-branch membership, and asset coverage. Reviewers emit `AgentFindings` only. Validate each review result with `python scripts/validate_agent_findings.py <findings.json> --request-packet <request-packet.json> --context <role-context.json> --base-context <contexts/normalized-context.json> --role-packet <review-role-packet.json> --draft-requirement <immutable-pending-draft>`. The same sole Synthesizer adjudicates them and writes the revised authoritative file. Every reviewer finding must receive a `reviewResolutions` record (`resolved`, `accepted-deviation`, or `rejected`); high-impact findings cannot remain `open` when the gate is accepted.

The final normalization must contain exactly one `findingsInputs` record for each of the nine schema roles: the three discovery roles, three focused-analysis roles, and three review roles. Each record carries a relative `findingsRef`, its file SHA-256, the parent RequestPacket `inputDigest`, and the exact `rolePacketRef` plus `rolePacketSha256` that produced it. The six context-aware records additionally carry their own RequirementSpec-relative projected or `full-fallback` `contextRef` and that file's SHA-256; first-round records must omit both fields. Bind the common authoritative base once through `normalization.baseContextRef`, `baseContextSha256`, and `baseContextCanonicalSha256`.

Run `python scripts/validate_requirement_spec.py <ui-requirement.json> --request-packet <request-packet.json> --check-findings-files --review-draft <immutable-pending-draft>` after synthesis and after every review-driven change. In this strict mode the validator runs every validator against the complete authoritative Requirement, then rehashes and deeply validates the authoritative base, all nine role packets, linked AgentFindings, role contexts, and all three Review Views against the immutable pending Draft. A Review View never replaces or relaxes complete-Requirement validation. The validator requires plugin-default role-card/Schema authority, exact role/source/output/context identities, canonical-reference closure or exact `full-fallback`, and every discovered local ID represented exactly once by `aliases` or `discardedLocalIds`. The final `UIRequirementSpec` format is unchanged; `--review-draft` is an external validation sidecar, not a new Requirement field. Fix all errors before presenting the user review. A pending spec may retain `null` target fields, empty `target.targetAssetPaths`, and an empty `assetPlan`; do not fabricate a destination merely to make the draft look buildable.
...
|-- request-packet.json
|-- inputs/                   # validated shortlist and on-demand Registry expansions
|-- agent-inputs/             # self-contained no-history packets and byte telemetry summaries
|-- findings/
|-- contexts/
|   |-- normalized-context.json # complete authoritative normalized base
|   `-- roles/                # verified role projections or full-fallback copies
|-- review-views/
|   |-- state-visual-review.review-view.json       # validated non-authoritative Draft View
|   |-- schema-feasibility-review.review-view.json # validated non-authoritative Draft View
|   `-- coverage-review.review-view.json           # validated non-authoritative Draft View
|-- ui-requirement.json
...
This skill owns `request-packet.json`, `inputs/`, `agent-inputs/`, `findings/`, `contexts/`, `review-views/`, and `ui-requirement.json`. Do not fabricate the downstream files. The planning/build workflow creates them only after user approval.
...
- all nine role packets validate, require `historyPolicy.mode: none` and `forkTurns: none`, contain no history/model override fields, and bind exact role/source/context/output identities;
- all three reviewer packets bind only their validated role-specific Review View rather than the complete Draft; each View has a complete canonical-ID dependency closure or an exact `full-fallback`, and the immutable full Draft remains available only to validators;

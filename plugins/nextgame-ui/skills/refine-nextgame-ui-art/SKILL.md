...
- Require the current accepted Requirement, verified baseline Bundle, normalized actual readback, and a separate actual art snapshot. For an unfinished formal-art development build, explicitly adopt `development-baseline/1` in a new request: retain every pending preview check and use its strict compiled/saved actual-state contract; this is not final verification. Preserve immutable baseline files; final Bundle and art evidence must not form circular hash references.

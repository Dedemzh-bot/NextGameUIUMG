...
## Development baseline resource deferrals

Use `development-baseline/2` only when the first art pass must close accepted Brush resource, DrawAs or mirroring values that are demonstrably different in the actual saved baseline. Existing `/1` requests retain their strict preview-only semantics. Create a new version 2 request/job; never relabel historical evidence. No Requirement, Bundle or Readback schema version changes.

Version 2 retains `structureCompletedAt`, exhaustive `pendingCheckIds` and `buildEvidence` from version 1, and requires these closed fields:

- `basicPropertyCheckIds`: exactly one original passed `key-properties` check for each asset. All assets also need passed schema/compile/save/widget-tree checks and real compile/save receipts. A deferred check cannot provide this coverage.
- `deferredPropertyChecks`: nonempty records of `check`, `reason: art-owned-property-closure` and nonempty `fields`. `check` is the exact original `{id,type,assetId,requirementRefs,claimIds}`; its type stays `key-properties`, status stays pending and claims must be accepted. The field record contains `assetId`, `elementId`, `nodeMappingId`, `widgetName`, exact `sourcePointer` and `sourceValue`, `expectedNativeValue`, `nativeProperty` (exactly `brush.resourceObject`, `brush.drawAs` or `brush.mirroring`), exact `actualSnapshotPointer` and `actualValue`.
- `pendingArtJudgments`: an explicit array, possibly empty, of `{elementId,sourcePointer,sourceValue,closureCheckIds}`. It preserves accepted element `textEffectJudgment: unresolved` with its original `textEffectReason`, pending `nineSliceMarginStatus` / `nineSliceStatus` strings, and the three original `staticAppearance`, `artMaterialCapability`, `artMaterialExecutionStatus` records for an explicit procedural material capability. Procedural intent does not invent a confirmed material path. Each original pending preview closure check remains unchanged. These records grant no native-property exemption.
- `primaryReview`: path/SHA-256 binding to the independently authored review below. In the compact job this is the existing review file's path; `init` only hashes it.

The validator independently enumerates the accepted in-scope element properties `defaultBrushResourceObject`, `drawAs` and `mirrorDesign` and resolves Bundle mappings to actual GameImages in the normalized readback and art snapshot. The declared resource fields must exactly equal all observed mismatches, preserving the native raw string or `refPath` representation. Source-to-native conversion is fixed: `defaultBrushResourceObject` → `brush.resourceObject`, `drawAs` → `brush.drawAs`, `mirrorDesign` → `brush.mirroring`; `MirrorHorizontal/MirrorVertical/MirrorBoth` become the actual native enum `Horizontal/Vertical/Both`, while `NoMirror` is unchanged. Other strings are not guessed. An unobserved property is not a demonstrated gap. Already matching resources, unrelated properties, unmatched IDs, stale source values and omitted gaps fail. This closed first implementation does not defer fonts, margins, geometry, variables, state or EntryClass.

The review uses closed definition `developmentBaselineReview` in `art-contract.schema.json`: `kind: development-baseline-art-deferral-review`, `version: 1`, `status: approved`, `reviewedBy: {role: primary-agent,id: <actual reviewer>}`, `reviewedAt`, exact `requirementSha256`, `bundleSha256`, `readbackSha256`, `snapshotSha256`, `contractSha256`, `reason: art-owned-property-closure`, `confirmedNonArtNativeResponsibilitiesVerified: true`, `notResultAcceptance: true`, and nonempty `notes`. `contractSha256` uses the existing canonical digest of the entire version 2 contract except `primaryReview`. The review follows the structural completion and both actual snapshots.

The primary coordinator must inspect the source-to-actual field audit and each remaining responsibility before writing that review. Production code does not generate or sign it. The declaration covers confirmed non-art native duties, while the explicitly listed unresolved artistic judgments remain open for the presentation-review stage. Do not turn unconfirmed shadow or nine-slice judgments into invented numeric requirements or treat defaults as art completion.

Only the internal development actual-state call receives the already validated exact deferred property IDs; those IDs cannot satisfy per-asset basic property coverage. The public final readback API has no deferral argument, and final Bundle, result acceptance and document gates still reject unfinished checks. Final closure preserves each original check's ID/type/asset/requirement refs/claim refs and requires passed status, including all deferred resource checks and all art-judgment preview checks. Source or actual changes invalidate the review and require new bound evidence.
